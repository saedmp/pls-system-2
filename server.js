...
setInterval(() => {
  http.get(`http://pls-system-2.glitch.me/`); ///تعديل مهم حط بدل 333م البروجكت
}, 280000);
...
const dev = ["470003692322553857"]; ///تعديل مهم حط ايدي حسابك
const admin = "=";
var owner = "470003692322553857"; // Owner Bot Mention /// تعديل مهم حط ايدي حسابكتعديل مهم ايدي حسابك
...
client.on("ready", () => {
  ...
  console.log(`saedsos`);
  console.log(`----------------`);
  console.log(`ON ${client.guilds.size} Servers '     Script By : saedsos  `);
  console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
  client.user.setActivity("=help | PLS : https://discord.gg/WRuEm4b " ); ///تعديل مهم غير كلمة هيلب وبرميوم اللي بدك اياه مثل اسم سيرفرك
...
client.on("message", message => {
  if (message.content.startsWith("=new")) {
    const reason = message.content
      ...
      .slice(1)
      .join(" ");
    if (!message.guild.roles.exists("name", "مشرف- <M O D S>"))
      return message.channel.send(
` لازم تسوي رتبة اسمها \`مشرف- <M O D S>\`.`
      );
    if (
    ...
    message.guild
      ...
      .then(c => {
        let role = message.guild.roles.find("name", "مشرف- <M O D S>");
...
client.on("message", zaid => {
  if (zaid.content === "=bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@470003692322553857>`, true) // تعديل مهم عدل هذا الرقم لايدي حسابك
      .setImage(
        ""
      )
...
let room = "693553265383571517"; //تعديل مهم ايدي روم عد الاعضاء والترحيب ، روم صوتي
...
client.on("guildCreate", guild => {
  var embed = new Discord.RichEmbed().setImage(
   ...
   ✽ **Support Server** [ • https://discord.gg/WRuEm4b • ]  ✽ `); 
...
client.on("message", message => {
 ...
 let channelc = {};
  ...
  const rebellog = client.channels.find("name", "معلومات-للبوت-عن-الهاك"), /// hack-log تعديل مهم سوي روم اسمو
